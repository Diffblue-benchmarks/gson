...
public class JsonStreamParserDiffblueTest {
  ...
  public void testNext_givenJsonStreamParserWithJsonIs42_thenReturnAsStringIs42()
    ...
    assertEquals(new BigDecimal("42"), actualNextResult.getAsBigDecimal());
    assertEquals('*', actualNextResult.getAsByte());
    JsonPrimitive actualAsJsonPrimitive = actualNextResult.getAsJsonPrimitive();
    assertSame(actualNextResult, actualAsJsonPrimitive);
  }
   ...
   *
   * <ul>
   *   <li>Given {@link StringReader#StringReader(String)} with {@link Boolean#FALSE} toString skip
   *       one.
  ...
  @ManagedByDiffblue
  @MethodsUnderTest({"JsonElement JsonStreamParser.next()"})
  public void testNext_givenStringReaderWithFalseToStringSkipOne_thenReturnAsStringIsE()
      throws JsonParseException, IOException {
    // Arrange
    StringReader reader = new StringReader(Boolean.FALSE.toString());
    reader.skip(1L);
    reader.read("\u0001\u0006\u0001\u0006".toCharArray(), 1, 3);

    // Act
    JsonElement actualNextResult = new JsonStreamParser(reader).next();

    // Assert
    assertTrue(actualNextResult instanceof JsonPrimitive);
    Number asNumber = actualNextResult.getAsNumber();
    assertTrue(asNumber instanceof LazilyParsedNumber);
    assertEquals("e", actualNextResult.getAsString());
    assertEquals("e", asNumber.toString());
    assertEquals('e', actualNextResult.getAsCharacter());
    JsonPrimitive actualAsJsonPrimitive = actualNextResult.getAsJsonPrimitive();
    assertSame(actualNextResult, actualAsJsonPrimitive);
  }
  ...
  public void testNext_givenStringReaderWithFalseToString_thenReturnNotString()
    ...
    assertEquals(Boolean.FALSE.toString(), actualNextResult.getAsString());
    JsonPrimitive actualAsJsonPrimitive = actualNextResult.getAsJsonPrimitive();
    assertSame(actualNextResult, actualAsJsonPrimitive);
  }
  ...
  public void testHasNext() {
    ...
    // Act and Assert
    assertTrue(new JsonStreamParser(reader).hasNext());
  }
